...
if is_train and not fast_debug and cv < 0:
    ...
    fileHandler = logging.FileHandler("{0}/{1}.match.log".format(logPath, logFile))
...
for _cv in cvs:
    ...
    if is_train:
        train_model(model, data, 50, batch_size,
                    './%smodel/jasonnetwork.bs%d.h%d.att_%s.drop%d.watt%d.b%d.l%d%s.match.model' % (
                        '' if cv < 0 else 'cv/',
