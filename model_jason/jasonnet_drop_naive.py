...
if is_train and not fast_debug and cv < 0:
    ...
    fileHandler = logging.FileHandler("{0}/{1}.naive.log".format(logPath, logFile))
...
class JasonNetwork(object):
    ...
    def __init__(self, batch_size, max_seq_len, embedding_size, hidden_size, vocab,
        ...
        if word_level_att:
            ...
            if self.word_level_att == "concat":
                ...
                self.W_enc_to_wattn2 = lasagne.utils.create_param(
                    lasagne.init.GlorotUniform(), (self.hidden_output_size, self.hidden_output_size), "W_enc_to_wattn2")
                wattn_parameters = [self.W_enc_to_wattn2]
    ...
    def word_level_attn_func(self, q1_hs, q1_hs_mask, q2_hs, q2_hs_mask, need_att=False):
        """Attention function
        Args:
        ...
        if _type == "general":
            if need_att:
                new_hid = T.tanh(T.dot(q1_hs, self.W_enc_to_wattn2).dimshuffle(0, 2, 1))
            else:
    ...
    def _build_predictor(self, en_out_q1_syms, en_out_q2_syms, en_q1_mask_sym, en_q2_mask_sym,
                         diff_q1_mask_sym, diff_q2_mask_sym, other_preds, leak_feaes=None, train=True):
        if train:
    ...
    def _build_word_att_predictor(self, en_out_q1_syms, en_out_q2_syms, en_q1_mask_sym, en_q2_mask_sym, train=True):
        ...
        q1_pred = q1_pred * en_q1_mask_sym
        q1_pred = (q1_pred).sum(axis=1, keepdims=True) / en_q1_mask_sym.sum(axis=1, keepdims=True)
        q2_pred = lasagne.layers.get_output(
            self.l_wp_pred_reshape, inputs=input_list_q2, deterministic=not train)
        q2_pred = q2_pred * en_q2_mask_sym
    ...
    def _build_word_att_cnn(self, en_out_q1_syms, en_out_q2_syms, en_q1_mask_sym, en_q2_mask_sym, train=True):
        if train:
            logger.debug(">>> Word Level Attention Predictor <<<")
        en_q1 = np.zeros((self.batch_size, self.max_seq_len,
                          self.hidden_output_size)).astype("float32")
        ...
        eval_list[en_out_q2_syms] = en_q2
        # att values
        att_matrix = self.word_level_attn_func(
            en_out_q1_syms, en_q1_mask_sym, en_out_q2_syms, en_q2_mask_sym, need_att=True)
        # arch for cnn
        if train:
    ...
    def build(self):
        ...
        # naive
        _, en_out_syms_train2, en_out_syms_test2, en_para_syms2 = self._build_encoder(en_in_syms)
        _, en_out_syms_train3, en_out_syms_test3, en_para_syms3 = self._build_encoder(en_in_syms)
        self.parameters += en_para_syms2
        self.parameters += en_para_syms3
        others_1_train = self._build_word_att_predictor(en_out_syms_train2[0], en_out_syms_train2[1], en_in_syms[1], en_in_syms[3], train=True)
        others_1_test = self._build_word_att_predictor(en_out_syms_test2[0], en_out_syms_test2[1], en_in_syms[1], en_in_syms[3], train=False)
        others_2_train = self._build_word_att_cnn(en_out_syms_train3[0], en_out_syms_train3[1], en_in_syms[1], en_in_syms[3], train=True)
        others_2_test = self._build_word_att_cnn(en_out_syms_test3[0], en_out_syms_test3[1], en_in_syms[1], en_in_syms[3], train=False)
        ...
        os = self._build_predictor(
            en_out_syms_train[0], en_out_syms_train[1], en_in_syms[1], en_in_syms[3], diff_q1_mask_sym, diff_q2_mask_sym,
            others_1_train + others_2_train, train=True, leak_feaes=leak_feaes)
        os_test = self._build_predictor(
            en_out_syms_test[0], en_out_syms_test[1], en_in_syms[1], en_in_syms[3], diff_q1_mask_sym, diff_q2_mask_sym,
            others_1_test + others_2_test, train=False, leak_feaes=leak_feaes)
...
def build_inputs(vocab, max_len, q1, q2, y, fast_debug=False):
    ...
    # if leak_f:
    #     return [x_q1, x_mask_q1.astype('float32'), x_q2, x_mask_q2.astype('float32'), x_q1_char, x_q2_char, x_mask_diff_q1, x_mask_diff_q2, x_leak.astype('float32'), y, weights]
    # return [x_q1, x_mask_q1.astype('float32'), x_q2, x_mask_q2.astype('float32'), x_q1_char, x_q2_char, x_mask_diff_q1, x_mask_diff_q2, y, weights]
...
for _cv in cvs:
    ...
    if is_train:
        train_model(model, data, 50, batch_size,
                    './%smodel/jasonnetwork.bs%d.h%d.att_%s.drop%d.watt%d.b%d.l%d%s.naive.model' % (
                        '' if cv < 0 else 'cv/',
        ...
        probas = predict(model, data, 4, batch_size,
                         './model/jasonnetwork.bs%d.h%d.att_%s.drop%d.watt%d.b%d.l%d.naive.model' % (batch_size, hidden_size, attn_type, int(drop_rate * 100), 1 if word_level_att else 0, 1 if balance else 0, 1 if leak_f else 0))
