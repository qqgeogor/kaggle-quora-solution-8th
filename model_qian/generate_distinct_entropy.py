...
def create_df_dict(paths,smooth=1.0,inverse=False,ngram='unigram'):
    K_dict = dict()
    c = 0
    start = datetime.now()
    sentences = []
...
def prepare_entropy(path,out,ngram='unigram'):
    ...
    with open(out, 'w') as outfile:
        ...
        for t, row in enumerate(DictReader(open(path), delimiter=',')): 
            if c%100000==0:
                print 'finished',c
            q1 = str(row['question1_distinct_%s'%ngram]).split(' ')
            q2 = str(row['question2_distinct_%s'%ngram]).split(' ')
            if ngram=='unigram':
...
prepare_entropy(path+'train_distinct_unigram.csv',path+'train_entropy_distinct_unigram.csv','unigram')
prepare_entropy(path+'test_distinct_unigram.csv',path+'test_entropy_distinct_unigram.csv','unigram')

prepare_entropy(path+'train_distinct_bigram.csv',path+'train_entropy_distinct_bigram.csv','bigram')
prepare_entropy(path+'test_distinct_bigram.csv',path+'test_entropy_distinct_bigram.csv','bigram')
