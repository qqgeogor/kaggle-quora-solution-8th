...
# model = read_emb(path+'glove.840B.300d.txt')from glove import Glove
from glove import Glove
model = Glove.load(path+'glove.glv')
...
def get_vec(paragraph,model):
    ...
    return model.word_vectors[word_ids]
...
def prepare_distinct(path,out,embedder):
    ...
    with open(out, 'w') as outfile:
        ...
        for t, row in enumerate(DictReader(open(path), delimiter=',')): 
            ...
            # print q1,"_______",q2
            a2 = [x for x in q1.split(' ') if x in model.dictionary]
            b2 = [x for x in q2.split(' ') if x in model.dictionary]
            # print a2,b2
                ...
                for i in range(len(a2)):
                    for j in range(len(b2)):
                        try:
                            ...
                            va = get_vec([worda],model).ravel()
                            vb = get_vec([wordb],model).ravel()
                            try:
                                # sim = cosine(model.transform_paragraph(worda),model.transform_paragraph(wordb))
                                sim = cosine(va,vb)
                            except:
                                sim=0.0
                            # vector_diff = model.transform_paragraph(worda)-model.transform_paragraph(wordb)
                            vector_diff = va-vb
...
prepare_distinct(path+'train.csv',path+'train_distinct_word_stats_selftrained_glove.csv',model)
prepare_distinct(path+'test.csv',path+'test_distinct_word_stats_selftrained_glove.csv',model)
