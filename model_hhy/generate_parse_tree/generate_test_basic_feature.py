...
if __name__ == '__main__':
    ...
    test = test[['question1','question2']]
    batch_size = 10000
    epoch = int(test.shape[0]*1.0/batch_size)+1
    for i in range(218,epoch):
        print('generate batch---{0}---'.format(i+1))
        st = i*batch_size
        et = (i+1)*batch_size
        if et>test.shape[0]:et = test.shape[0]

        cur_batch = generateTreeAndPos(st, et, test)
        cur_batch = generateTreeFeature(cur_batch)

        pd.to_pickle(cur_batch, out_path + "test_DepTree{0}.pkl".format(i))
